...
<template>
    ...
    <div class="sm:rounded-lg" :class="{'overflow-x-scroll':filteredLogistics.length > 0}">
        <TableLayout
            :headers="headers"
        ...
        >
            <template #tbody>
                    <tr class="border-b item" v-for="(logistic,index) in filteredLogistics" :key="logistic.id">
        ...
        <template v-else>
            <p>Nothing to Search logisitcs!!</p>
        </template>
    </div>
</template>
