...
<template>
    <div class="flex gap-5 items-center font-medium">
        <p>
            Sort By
            <span
                class="cursor-pointer inline-block font-bold capitalize"
                @click="sorting(sortBy)"
                >{{ sortBy }}</span
            >
            <i
        ...
        <p>
            ...
            <i
                class="fa-solid ms-1 fa-angle-up transition-transform duration-700"
                :class="{
                    'rotate-180': directionForTime === 'asc',
                    'rotate-270': directionForTime === 'desc',
                }"
            ></i>
        </p>
    </div>
